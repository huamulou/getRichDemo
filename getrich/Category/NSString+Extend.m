...
- (NSMutableAttributedString *)attributedStringWithChineseFontSize:(CGFloat)cfontSize
    ...
    return [self attributedStringWithChineseFontSize:cfontSize withNumberAndLetterFontSize:nlFontSize withChineseFontColor:nil withNALFontColor:nil withLineSpacing:lineSpacing];
...
- (NSMutableAttributedString *)attributedStringWithChineseFontSize:(CGFloat)cfontSize
    ...
    if ([self length] > 0) {
        ...
        return [self attributedStringWithChineseFont:cFont withNumberAndLetterFont:nlFont withChineseFontColor:cColor withNALFontColor:nlColor withLineSpacing:lineSpacing];
...
- (NSMutableAttributedString *)attributedStringWithChineseFont:(UIFont *)cfont
                                       withNumberAndLetterFont:(UIFont *)nlFont
                                               withLineSpacing:(CGFloat)lineSpacing {
    return [self attributedStringWithChineseFont:cfont withNumberAndLetterFont:nlFont withChineseFontColor:nil withNALFontColor:nil withLineSpacing:lineSpacing];
    ...
    if ([self length] > 0) {
        ...
        void (^addAttribute)(NSInteger type, NSInteger rangeStart, NSInteger length) = ^(NSInteger type, NSInteger rangeStart, NSInteger length) {
            if (type == typeChinese) {
                ...
                if (cColor) {
                    [attributedStr addAttributes:@{NSForegroundColorAttributeName : cColor} range:NSMakeRange(rangeStart, length)];
                }
            } else {

                [attributedStr addAttributes:@{NSFontAttributeName : nlFont} range:NSMakeRange(rangeStart, length)];
                if (nlColor) {
                    [attributedStr addAttributes:@{NSForegroundColorAttributeName : nlColor} range:NSMakeRange(rangeStart, length)];
                }
            }
...
- (NSMutableAttributedString *)attributedStringFromStingWithFont:(UIFont *)font {
    return [self attributedStringFromStingWithFont:font withLineSpacing:0];
}
